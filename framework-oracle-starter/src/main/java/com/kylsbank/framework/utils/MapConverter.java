...
public class MapConverter {
    ...
    public static void decodeByField(Object obj, String... fieldNames) {
        ...
        decodeByField(obj.getClass(), obj, fieldNames);
    }

    /**
     * 解码UTF8
     */
    private static void decodeByField(Class c, Object obj, String... fieldNames) {
        Field[] fileds = c.getDeclaredFields();
        for (Field f : fileds) {
            if (Modifier.isStatic(f.getModifiers())) { // 跳过静态属性
                continue;
        ...
        // 转化继承属性
        if (c.getSuperclass() != null && !c.getSuperclass().getName().equals("java.lang.Object")) {
            decodeByField(c.getSuperclass(), obj, fieldNames);
        }
    }
